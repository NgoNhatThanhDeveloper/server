...
export const requestEmailService = (req, res) => {
    repository
        ...
        .then(() => {
            return res.json({
                success: true,
                result: "Yêu cầu thành công, kiểm tra mã xác thực trong email của bạn. Mã xác thực có thời hạn là 5 phút",
            });
        })
        .catch((error) => {
            return res.json({ success: false, result: error.message });
...
export const replaceEmailService = (req, res) => {
    repository
        ...
        .then(() => {
            return res.json({
                success: true,
                result: "Yêu cầu thành công, email mới được xác thực,từ giờ các yêu cầu  sẽ dựa trên email này để xác thực",
            });
        })
        .catch((error) => {
            return res.json({ success: false, result: error.message });
